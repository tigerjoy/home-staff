-- Update Member Access Functions to Check Active Status
-- This migration updates helper functions to check for active member status
-- and adds new functions for permission checking

-- Update is_household_member to check for active status
-- Keep the old function for backward compatibility but add a new one for active members
...
-- Update is_household_admin to check for active status
...
-- Function to get member's effective permissions
-- Returns permissions array combining role defaults and custom overrides
...
BEGIN
  ...
  v_permissions := public.get_member_permissions(p_household_id, p_user_id);
...
-- Function to get member status and role
